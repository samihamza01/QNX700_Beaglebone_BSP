...
#ifdef VARIANT_omap4
    sprintf(shm_lock_name, "/i2c%d_debug", dev->intr);
	/* Open/Create the shared memory object */
	shm_fd = shm_open(shm_lock_name, O_CREAT| O_RDWR, 0777 );
	if( shm_fd < 0 ) {
		fprintf(stderr, "shm_open: Creating i2c shm object failed\n");
	} else {
	    /* Set the memory object's size */
        if( ftruncate( shm_fd, sizeof( *dev->debug_en ) ) == -1 ) {
            fprintf( stderr, "ftruncate: Could not set memory object size\n" );
            close(shm_fd);
            dev->debug_en = NULL;
        } else {	
	        /* Map the memory object */
            dev->debug_en = mmap( 0, sizeof( *dev->debug_en ), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0 );
            if( dev->debug_en == MAP_FAILED ) {
                fprintf( stderr, "mmap: mmap failed for i2c shared object\n");
                close(shm_fd);
                dev->debug_en = NULL;
            }
        }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/i2c/omap35xx/init.c $ $Rev: 756950 $")
#endif
