...
int omap3_init_edma(omap3_spi_t *omap3)
{
	...
	if (omap3->edma) {
			...
			if (omap3->dmabuf != MAP_FAILED) {
				if (mem_offset(omap3->dmabuf,NOFD,1,&offset,0) != EOK){
					return -1;
				}
				omap3->pdmabuf = (uint32_t)offset;
			}
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/spi/dm816x/edma.c $ $Rev: 808731 $")
#endif
